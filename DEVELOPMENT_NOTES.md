...
드래그로 다른 위젯의 슬롯을 "claim"하면(`claim_edge=True`) 그 순간부터 상대 스트립을 장애물 목록에서 지우고 그 자리를 자기 것으로 스윕한다. 문제는 상대가 아직 물리적으로 비키지 않았는데 이쪽이 먼저 그 자리를 차지하면 두 스트립이 겹친다. 해결은 `_observe_once`에서 매 관찰 주기(약 1.5초)마다 상대가 **실제로 자기 claim을 내려놓은 순간**(`target.claiming`이 거짓이 되는 순간)을 확인해 그때 비로소 `claim_edge`를 자동으로 끄는 것이다. 그 전까지는 계속 상대를 무시한 채로 스윕하므로 UI상으로는 즉시 반응하는 것처럼 보이지만, 실제 자리싸움은 상대가 물러난 뒤에만 확정된다.
