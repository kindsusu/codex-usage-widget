...
def test_tray_commands_forward_without_calling_tk() -> None:
    ...
    callbacks = TrayCallbacks(
        toggle_desktop=lambda: events.append("desktop"),
        toggle_mini=lambda: events.append("mini"),
        toggle_taskbar=lambda: events.append("taskbar"),
        show_details=lambda: events.append("details"),
        desktop_checked=lambda: False,
        mini_checked=lambda: True,
    ...
    controller._on_toggle_desktop(None, None)
    controller._on_toggle_mini(None, None)
    controller._on_toggle_taskbar(None, None)
    controller._on_refresh(None, None)

    assert events == ["details", "desktop", "mini", "taskbar", "refresh"]
