...
- **Taskbar indicator unavailable**: the indicator targets the primary horizontal Windows 11 taskbar. It sweeps the free runs from the left edge (an 8 logical-pixel margin) and takes the first one that fits, sitting flush against a sibling usage strip when there is one; anything that really occupies the leading edge, such as a Widgets surface, is measured and avoided whatever its control type. If no 161-pixel run is free there it falls back to the space before the notification area, and for vertical/unsupported taskbars, secondary-monitor taskbars, or Explorer transitions it falls back to the desktop widget temporarily without changing saved visibility settings. Windows 10 has not been tested on a live device.
