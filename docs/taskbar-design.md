...
좌우 배치는 **2026-09-14 사용자 지시**로 바뀌었다. **Codex 마크가 왼쪽, 사용량 백분율이 오른쪽**이다. 위 HTML 시안은 그 이전 좌우 순서를 담고 있으므로, 좌우 순서에 한해서는 이 문서가 정본이다. 치수·팔레트·상호작용 나머지는 그대로다.

## 크기와 배치

- 왼쪽에 세로 중앙 정렬한 `38 × 44` logical px Codex 버튼(`y=1`, `x=0..38`)을 둔다.
- 그 오른쪽으로 5px 간격을 두고 `154 × 46` logical px 사용량 버튼(`x=43..197`)이 온다. 안쪽 여백은 가로 8px·세로 4px이다. 따라서 기본 선호 폭은 `197 × 46` logical px이다.
- 사용량 버튼 안의 두 행: `31px` 라벨, `6px` 간격, 기본 `59px` 막대, `6px` 간격, `36px` 값으로 배치한다. 모든 x는 사용량 버튼 왼쪽 모서리 기준이다.
- 각 행의 grid 높이는 19px(`y=4..23`, `y=23..42`)이다. 라벨은 11px 보통 굵기, 백분율은 14px semibold와 tabular 숫자를 사용한다.
- 진행 막대는 높이 4px, 완전한 둥근 모서리다.
- Codex 아이콘은 19px 단색이며, 버튼 오른쪽 아래에 5px 초록 상태 점을 둔다.
...
- 왼쪽 Codex 아이콘의 왼쪽 클릭은 [표시 설정 패널](desktop-card-design.md)을 열고, 오른쪽 클릭은 고급 네이티브 컨텍스트 메뉴를 연다.
